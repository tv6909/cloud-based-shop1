...
export function Shopkeepers() {
  ...
  const handlePayment = async (shopkeeperId: number) => {
    ...
    try {
      ...
      // Update shopkeeper balance in database
      const { error: updateError } = await supabase
        .from("shopkeepers")
        .update({
          current_balance: Math.max(
            0,
            (shopkeeper.balance < 0 ? Math.abs(shopkeeper.balance) : shopkeeper.totalPendingAmount) - amount,
          ),
      ...
      if (updateError) {
        console.error("[v0] Failed to update shopkeeper balance in database:", updateError)
      }
      ...
      if (existingShopkeeper) {
        const updatedShopkeeper = {
          ...existingShopkeeper,
          current_balance: Math.max(0, (existingShopkeeper.current_balance || 0) - amount),
          updated_at: new Date().toISOString(),
        }
      ...
      window.dispatchEvent(
        new CustomEvent("shopkeeper-payment-made", {
          detail: {
            ...
            newBalance: Math.max(0, totalDue - amount),
